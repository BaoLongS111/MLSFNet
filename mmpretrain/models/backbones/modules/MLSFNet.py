...
class SKChannelSelectorHybrid(nn.Module):
    ...
    - base_logits: [K, C] learnable per-branch per-channel bias (robust to weight_decay).
    - mlp_delta: zero-initialized last layer -> starts uniform (safe).
    - desc: global descriptor computed from fused feature sum, LN-normalized.
    """
    def __init__(self, channels: int, num_branches: int, reduction: int = 4, tau: float = 1.5):
        ...
        # Zero-init => starts uniform and preserves baseline at init
    ...
    def forward(self, feats):
        ...
        # Descriptor from fused feature (sum of branches) improves stability vs per-branch pooling
        u = feats[0]
        for i in range(1, self.num_branches):
            u = u + feats[i]
        s = u.mean(dim=(2, 3))             # [B, C]
        s = F.layer_norm(s, (C,))          # normalize over channels
...
class MLSFNetOp(BaseModule):
    """
    Multi-scale depthwise conv operator (v2 backbone + small innovations).
    Branches (core):
      - DW 3x3
      - DW 5x5
      - DW 7x7

    7x7 branch augmentation (re-parameterizable, gated scalars init 0):
      - anisotropic DW (1xk + kx1)
      - dilated DW 3x3

    Additional inductive bias:
      - high-frequency residual: (x - avgpool(x)) scaled by alpha_hp (init 0)

    Lightweight channel mixing (conv-space):
      - low-rank pointwise mixer (C -> r -> C), up-proj zero-initialized.
    """
    def __init__(
        self,
        in_features: int,
        ...
        # High-frequency residual (no extra conv params)
...
class MLSFNet(BaseModule):
    """
    Token-space adapter: tokens -> conv-space -> tokens.

    `use_norm_gating` options:
    - True (default): x = LN(x) * gamma + x * gammax
      This keeps a near-identity path and was stable for your v2 runs.
    - False: skip this internal gating.

    Regularization:
    - drop_path_prob is derived from `drop` (default: 0.05 when drop=0.1).
    """
    def __init__(
        self,
        in_dim: int,
